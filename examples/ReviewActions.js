...
import * as HostInfo from "../src/utils/HostInfo";
import * as ResourcePaths from "../src/utils/ResourcePaths";
...
export const fetchInReviewProblems = (coder_id = -1) => dispatch => {
  ...
  axios({
    ...
    .then(response =>
      dispatch({
        type: ActionTypes.FETCH_IN_REVIEW_PROBLEMS_FULFILLED,
        payload: response.data
      })
    )
    .catch(err =>
      dispatch({
        type: ActionTypes.FETCH_IN_REVIEW_PROBLEMS_REJECTED,
        payload: err
      })
    );
};
...
export const completeReview = data => dispatch => {
  ...
  axios({
    ...
    .then(response =>
      dispatch({
        type: ActionTypes.UPDATE_REVIEW_QUEUE_FULFILLED,
        payload: response.data
      })
    )
    .catch(err =>
      dispatch({ type: ActionTypes.UPDATE_REVIEW_QUEUE_REJECTED, payload: err })
    );
  ...
  axios({
    ...
    .then(response =>
      dispatch({
        type: ActionTypes.UPDATE_IN_REVIEW_PROBLEMS_FULFILLED,
        payload: response.data
      })
    )
    .catch(err =>
      dispatch({
        type: ActionTypes.UPDATE_IN_REVIEW_PROBLEMS_REJECTED,
        payload: err
      })
    );
};
...
export const fetchReviewQueue = (coder_id = -1) => dispatch => {
  ...
  axios({
    ...
    .then(response =>
      dispatch({
        type: ActionTypes.FETCH_REVIEW_QUEUE_FULFILLED,
        payload: response.data
      })
    )
    .catch(err =>
      dispatch({ type: ActionTypes.FETCH_REVIEW_QUEUE_REJECTED, payload: err })
    );
};
