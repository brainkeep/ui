...
export const changeNewQuestionName = question_name => dispatch => {
  dispatch(
    {type: ActionTypes.CHANGE_NEW_QUESTION_NAME, payload: question_name});
};

export const changeNewQuestionUrl = question_url => dispatch => {
  dispatch({type: ActionTypes.CHANGE_NEW_QUESTION_URL, payload: question_url});
};

export const saveNewQuestion = (coder_id, question_name, question_url, default_set_id) =>
  dispatch => {
    ...
    axios({
      ...
      .then(response => dispatch(
        {type: ActionTypes.SAVE_NEW_QUESTION_FULFILLED, payload: response}))
      .catch(err => dispatch(
        {type: ActionTypes.SAVE_NEW_QUESTION_REJECTED, payload: err}))
      .then(() => dispatch({type: ActionTypes.CLOSE_HEADER_DIALOG}));
  };
